...
NLL <- function(ytrue, yhat) {
  return(-sum(log(yhat^ytrue * (1-yhat)^(1-ytrue))))
}
...
train <- function(X, y, l_rate, epochs) {
  ...
  for (epoch in 1:epochs) {
    ...
    message(
      paste0(
        "Iteration ", epoch ,"/",epochs," | NLL = ", round(NLL_epoch, 5),"; MSE = ", round(MSE_epoch, 5)
      )
    )
  }
...
```{r}
coef_model <- train(X = X, y = y, l_rate = 0.01, epochs = 10)
```
